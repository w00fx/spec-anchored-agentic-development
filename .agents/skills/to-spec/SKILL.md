...
1. Identify the capability (the argument, or the session's subject). Resolve
   its one stable logical-contract entrypoint at
   `specs/<capability>/<capability>.md`, using
   `spec-templates/capability-spec.md`. The entrypoint may later declare
   companion files without creating another authority.
...
references it. Then invoke `spec-to-tickets`.
...
mirrors travel in that same PR. Downstream (`spec-to-tickets` and workers) refuses drafts.
