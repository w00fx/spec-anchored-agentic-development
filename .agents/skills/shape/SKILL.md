...
- **An idea or ongoing discussion** → interview toward a capability
  spec (the `to-spec` skill writes it).
...
- **An existing spec** → grill-back: interrogate the document for
  ambiguity and holes (the `to-spec` skill applies the refinements).
...
The spec file is written once, at the end, by the `to-spec` skill —
...
Before handing off to the `to-spec` skill, interrogate the assembled summary
...
- **Spec modes (create or refine): do NOT write or edit the spec
  ...
  open questions if ambiguity remains — then hand off: **invoke the `to-spec` skill to write the file.** Unresolved ambiguity belongs in the
...
The human is the source of business truth: this skill structures the
interview; the `to-spec` skill materializes it; nothing is invented on the
way.

Portability: this skill's body works as a standalone prompt — paste
