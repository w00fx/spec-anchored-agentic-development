      ...
      if [ ! -d "$ROOT/$CAP" ] && [ ! -d "specs/$CAP" ]; then
        {
          ...
          echo "Spec-first: write the capability spec first (specs/$CAP/$CAP.md — use the `shape` → `to-spec` skills), then implement."
          echo "If '$CAP' is not a capability (an entity or layer folder), it fails package-by-feature — see .agents/rules/package-by-feature.md."
        } >&2
        exit 2
      fi
      ;;
